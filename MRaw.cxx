...
//        printf(" hit %d, (col,row)=(%3d, %3d), (u,v)=(%.1f, %.1f), Npixels=%d, testMult=%d\n", iHit, (int)TheCol, (int)TheRow, u, v, Npixels_in_Hit, testMult);
    ...
    if(fVerbose) cout << "Tracker has " << nPairs << " track-pairs reconstructed" << endl;
    for( Int_t iTrack=1; iTrack<=nPairs; iTrack++ ) { // loop on tracks
      // not ready yet
      //hTrackMapTel[iPlane-1]->Fill( pairList[iTrack]s, tPlane->PlaneToTracker(posInPlane)(1), aTrack->GetNumber());

      if(fVerbose) printf("MRaw::DisplayHits2D  pl %d, track[%d] ...\n", iPlane, iTrack );
...
void MRaw::SitrineoCumul( Int_t nEvents, Int_t lastPlaneOfFirstTracker)
{
  ...
  //Double_t seedN;
  Int_t nPairs;
  trackpair_t pairList[100]; // maximum hundred trackpair reconstructed
  ...
  TH1F *hTrackPairAngle12 = new TH1F( "hTrackPairAngle12", "Track12 angle; angle12 [rad]", 62, -3.14, 3.14);
  TH1F *hTrackPairAngle34 = new TH1F( "hTrackPairAngle34", "Track34 angle; angle34 [rad]", 62, -3.14, 3.14);
  TH1F *hTrackPairDX12 = new TH1F( "hTrackPairDX12", "Track 12 #Delta X; #Delta X [cm]", 100, -2, 2);
  TH1F *hTrackPairDY12 = new TH1F( "hTrackPairDY12", "Track 12 #Delta Y; #Delta Y [cm]", 100, -2, 2);
  TH1F *hTrackPairDX34 = new TH1F( "hTrackPairDX34", "Track 34 #Delta X; #Delta X [cm]", 100, -2, 2);
  TH1F *hTrackPairDY34 = new TH1F( "hTrackPairDY34", "Track 34 #Delta Y; #Delta Y [cm]", 100, -2, 2);
  TH1F *hTrackPairDAngle = new TH1F( "hTrackPairDAngle", "Track tilt; #delta angle [rad]", 62, -3.14, 3.14);
  TH1F *hTrackPairMomentum = new TH1F( "hTrackPairMomentum", "Track momentum; p [MeV/c]", 50, 0, 50);
  ...
  for( Int_t iEvt=0; iEvt < nEvents; iEvt++) {
    ...
    for( Int_t iTrack=1; iTrack<=nPairs; iTrack ) { // loop of track-pairs
      hTrackPairDX12->Fill(pairList[iTrack].vector12_X);
      hTrackPairDY12->Fill(pairList[iTrack].vector12_Y);
      hTrackPairDX34->Fill(pairList[iTrack].vector34_X);
      hTrackPairDY34->Fill(pairList[iTrack].vector34_Y);
      hTrackPairAngle12->Fill( atan(pairList[iTrack].vector12_Y/pairList[iTrack].vector12_Z) );
      hTrackPairAngle34->Fill( atan(pairList[iTrack].vector34_Y/pairList[iTrack].vector34_Z) );
      hTrackPairDAngle->Fill( pairList[iTrack].angle);
      hTrackPairMomentum->Fill( pairList[iTrack].momentumXY);
  ...
  if (g) {
    ...
    ccumultrack = new TCanvas("ccumultrack", "Track properties", 50, 50,800,700);
  }
  ccumultrack->Clear();
  ccumultrack->UseCurrentStyle();
  label->DrawPaveLabel(0.3,0.97,0.7,0.9999,canvasTitle);
  ...
  hTrackPairDY12->Draw();
  pad4->cd(3);
  hTrackPairAngle12->Draw();
  pad4->cd(4);
  hTrackPairDX34->Draw();
  pad4->cd(5);
  hTrackPairDY34->Draw();
  pad4->cd(6);
  hTrackPairAngle34->Draw();
  pad4->cd(7);
  hTrackPairDAngle->Draw();
  pad4->cd(8);
  ...
  hTrackPairAngle12->Write();
  hTrackPairDX34->Write();
  hTrackPairDY34->Write();
  hTrackPairAngle34->Write();
  hTrackPairDAngle->Write();
...
void MRaw::SitrineoAnalysis( Int_t lastPlaneOfFirstTracker, Int_t &nPairs, trackpair_t* pairList)
{
  ...
  for( Int_t iTrack=1; iTrack<=tTracker->GetTracksN(); iTrack++) { // loop on 1st tracks // recup t from 1st tracker
	...
	cout << "First Track " << plane1 << endl ;
        cout << "Second Track " << plane2 << endl ;
      	...
      	pairList[nPairs].slope12 = slope1(0);
      	pairList[nPairs].slope34 = slope2(0);
      	double angle = acos( ( (pairList[nPairs].slope34)*(pairList[nPairs].slope12) + 1 )/(sqrt(1+pow(pairList[nPairs].slope34,2) ) * sqrt(1+pow(pairList[nPairs].slope34,2) ) ) ) ;
  ...
  if(fVerbose) {
    ...
    for (size_t ip = 0; ip < nPairs; ip++) {
      printf( "  %d: track1=%d + track2=%d, slope1=%.2e, slope2=%.2e, momentum=%.2e\n", ip, pairList[nPairs].firstTrackID, pairList[nPairs].secondTrackID, pairList[nPairs].slope12, pairList[nPairs].slope34, pairList[nPairs].momentumXY);
    }
  }
...
void MRaw::SitrineoAnalysisFromHits( Int_t lastPlaneOfFirstTracker, Int_t &nPairs, trackpair_t* pairList)
{
  ...
  for( int i4 = 0 ; i4 < Plane4->GetHitsN() ; i4++){
    ...
    for( int i3 = 0 ; i3 < Plane3->GetHitsN() ; i3++){
      ...
      if( -0.6 < deltaX && deltaX < 0.6 && -0.2 < deltaY && deltaY < 0.70 ){ // cut from 2019  -0.6   0.6   // -0.2  0.7
      //if( -2 < deltaX && deltaX < 2 && -2 < deltaY && deltaY < 2 ){
	vector34_X.push_back(deltaX) ;
	vector34_Y.push_back(deltaY) ;
	vector34_Z.push_back(deltaZ) ;
        id_hit3.push_back(i3);
        id_hit4.push_back(i4);
      	nvector34++;
      }
    }
  ...
  for( int i2 = 0 ; i2 < Plane2->GetHitsN() ; i2++){
    ...
    for( int i1 = 0 ; i1 < Plane1->GetHitsN() ; i1++){
      ...
      if( 0.3 < alpha12 && alpha12 < 0.9 && -1.3 < deltaX && deltaX < 1.3 ){ // cut from 2019
      //if( -1.5 < alpha12 && alpha12 < 1.5 && -2 < deltaX && deltaX < 2 ){
	vector12_X.push_back(deltaX) ;
	vector12_Y.push_back(deltaY) ;
	vector12_Z.push_back(deltaZ) ;
        id_hit1.push_back(i1);
        id_hit2.push_back(i2);
      	nvector12++;
      }
    }
  } //end loop on sensors 1-2

  printf(" SITRINEO analysis found %d vectors from plane 1-2 and %d vectors from 3-4\n", nvector12, nvector34);

  //pairing of vectors to get full track
  if( nvector34 != 0 && nvector12 != 0 ){ // test there is vectors
    for( int i34 = 0 ; i34 < nvector34 ; i34++){ // loop on vectors 3-4
      for( int i12 = 0 ; i12 < nvector12 ; i12++){ // loop on vectors 1-2
	...
	cout << "== pair " << nPairs <<" of tracks possible ==" << endl ;
        cout << "   angle [rad] : " << angle << " ==" << endl ;
	cout << "   momentum [MeV/c] : " << momentum << " ==" << endl ;

        pairList[nPairs].hit1 = id_hit1[i12];
        pairList[nPairs].hit2 = id_hit2[i12];
        pairList[nPairs].hit3 = id_hit3[i34];
        pairList[nPairs].hit4 = id_hit4[i34];
        pairList[nPairs].vector12_X = vector12_X[i12];
        pairList[nPairs].vector12_Y = vector12_Y[i12];
        pairList[nPairs].vector12_Z = vector12_Z[i12];
        pairList[nPairs].vector34_X = vector34_X[i34];
        pairList[nPairs].vector34_Y = vector34_Y[i34];
        pairList[nPairs].vector34_Z = vector34_Z[i34];
        pairList[nPairs].angle = angle;
        pairList[nPairs].momentumXY = momentum;
        nPairs++;
      } // end loop on vectors 1-2
    } // end loop on vectors 3-4
  } // end test there is tracklets
  else { cout << "== No pairing of tracks possible ==" << endl ; }
